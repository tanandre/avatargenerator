<template>
  <g>
    ...
    <use :x="`${startX}`" :y="`${startY}`" :xlink:href="`#head0${head.type.value}`" fill="rgba(0,0,0,0.2)"/>
    <use :x="`${startX}`" :y="`${startY}`"  :xlink:href="`#headSmaller0${head.type.value}`" :fill="`${head.color}`"/>
</g>
</template>
<script>
